...
- 이미지 파일이 너무 크면 파일로 배포하기가 어려운데, 도커에서 공식적으로 사용하는 도커 허브(이미지 저장소)를 사용해서 클라우스 서비스처럼 명령어를 통해서 인터넷만 된다면 어디서든 이미지를 등록하고 다운받을 수 있다.
- Docker Private Registry를 통해서 사용자가 직접 이미지 저장소를 만들 수 있다.

```
- tag 명령어를 사용해서 이미지의 이름을 추가 같은 이미지를 가르킨다.
docker tag [기존의 이미지 이름] [생성될 이미지 이름]
```
...
- 도커 허브에서 비공개로 설정할 경우 해당 계정으로만 이미지를 내려받을 수 있지만 도커 허브 저장소의 Collaborators 메뉴에서 사용자를 추가할 경우 이미지 저장소의 pull, push 권한을 얻을 수 있다.
- 사용자 별이 아닌 조직, 팀별로도 생성이 가능한데 도커 허브 저장소의 Organizations 메뉴에서 조직또는 팀을 생성해서 이미지를 관리할 수 있다.

#### 도커 허브 저장소 Webhooks

- 도커 허브 저장소의 이미지가 push 되었을 경우 특정 URL HTTP 요청을 하는 기능이며, 저장소에 추가된 이미지를 서버에 배포하는 애플리케이션을 작성할떄 활용하면 유용하다.
...
- 도커파일은 빌드 시 줄마다 레이어를 생성하고 레이어 생성 시 이미지를 빌드 하기위해서 컨테이너를 임시로 생성해서 컨테이너를 커밋하는 방식으로 이미지를 생성한다. 또한 캐시가 남아있는 경우 도커빌드중 자동으로 캐시를 읽어서 불필요한 명령어를 재실행하지 않은 장점이 있다. 다만 이런 캐시가 필요하지 않은 경우도 존재하는데 git clone같은 명령어를 사용해서 빌드할때는 명령어를 무조건 실행해야해서 캐시기능을 막아줘야한다. 이런 경우엔 옵션으로 --no-cache 옵션을 추가하면 처음부터 재빌드를 진행한다. 캐시로 사용할 이미지를 지정할 수도 있는데 --cache-from [이미지] 로 하면 해당 이미지를 캐시로 사용한다.
- 이미지가 빌드 중 오류가 발생했을 땐 레이어 생성을 위해 생성된 컨테이너가 임시로 남겨되며, 이미지또한 <none>:<none>으로 생성되서 삭제해줘야한다.
...
| 명령어 | 설명                                                                                     |
| ------ | ---------------------------------------------------------------------------------------- |
| ENV    | Dockerfile에서 사용할 환경변수 지정, 지정 후 ${ENV_NAME} 또는 $ENV_NAME 형태로 사용      |
| VOLUME | 빌드된 이미지를 컨테이너로 생성할 경우 호스트와 공유할 컨테이너 내부의 디렉토리를 설정   |
| ARG    | build 명령어 실핼 시 추가로 입력받아 Dockerfile 내 변수 값을 설정                        |
| USER   | 컨테이너 내에서 사용될 사용자의 계정이나 이름을 설정, 미설정시 기본적으로 root 계정 사용 |
...
| 명령어      | 설명                                                                                                                           |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------ |
| ONBUILD     | 빌드된 이미지를 기반으로 다른 Dockerfile로 생성될 때 해당 명령어가 실행된다.                                                   |
| STOPSIGNAL  | 컨테이너가 정지될 때 시스템 콜의 종류를 지정                                                                                   |
| HEALthCHECK | 이미지로 생성된 컨테이너 안의 애플리케이션 상태를 체크하도록 설정, --interval 옵션을 통해 상태를 체크하는 주기를 설정가능하다. |
| SHELL       | 사용할 셸을 지정                                                                                                               |
...
#### 도커 데몬(Docker Daemon) 참조블로그

[docker 데몬](https://rmcodestar.github.io/docker/2018/07/25/docker-daemon/)

#### 도커의 실제 위치는?

```
-- which 명령어로 도커 명령어의 위치를 확인 가능하다. 하지만 WSL의 경우 아래 경로가 나타나지만 접근은 따로 불가능하며, 따로 Mount 설정을 해줘야 한다.
# which docker
/usr/bin/docker
```

```
-- 실행 중인 도커 프로세스를 확인하면 /usr/bin/docker와 다르다.
# ps aux | grep docker
```

이와 같은 이유는 명령어는 /usr/bin/docker에서 실행되고, /usr/bin/dockerd 에서 파일이 실행되기 때문이다.
